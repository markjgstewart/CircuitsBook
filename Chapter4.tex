...
In summary, our nodal analysis procedure:

\begin{enumerate}
\item Identify unique nodes. Assign one node to be the reference node. It's voltage will of course be zero compared to itself.
\item Create a variable to represent the voltage difference for each node with respect to a the reference node.
\item write down KCL, $\Sigma I_{in}=0$ for each node, but not the reference node. 
\item Count equations and unknowns. Think.
\item Put into matrix form and solve.
\end{enumerate}
...
Let's follow the procedure while making some observations:
\begin{enumerate}
...
\end{tikzpicture}
\caption{Loop currents are indicated in blue. Each loop has a current through it.}
\end{center}
\end{figure}
...
\begin{itemize}
\item \textbf{First Tricky Thing:} Getting the signs right. Proceed around each loop in clockwise direction. If you move from - to +, then add that voltage. If you move from + to - then subtract it. Recall that positive currents flow through resistors from + to - voltage.
\item \textbf{Second Tricky Thing:} Handling components that have more than one loop current passing through them. The 3 Ohm resistor, for example, has $I_1$ passing through it from C to B and $I_2$ passing through it the other direction from B to C.
\end{itemize}
...
Replace the 7V source with a 3A current source. This causes a little trouble because we no longer know a good expression for the voltage from B to D. No worries - just assign it a variable, like $V_1$ \footnote{Do you see the similarity to nodal analysis?}. 
\par
But, we now have one too many unknowns \dots we need another equation. Use information about the current source. The current source forces the current from B to D to be 3A. Looking at the loop currents, we see the current from B to D is $I_3-I_2$, so we write $I_3-I_2=3 A$.
...
\BI{Many engineering systems can be approximated as linear systems. Even if the system is profoundly not linear, it might be linear over a range of inputs.}\vspace{5mm}
